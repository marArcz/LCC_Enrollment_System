    ...
    public partial class SubjectsTabComponent : CustomTab
    {
        ...
        public void LoadRows(bool movePagination = false)
        {
            ...
            if (dir == FetchDirection.forward)
            {
                ...
                rows = db.Subjects
                    ...
                    .Where(s => s.id > lastId && (s.Name.Contains(textBoxSearch.Text) || s.To.Contains(textBoxSearch.Text) || s.Days.Contains(textBoxSearch.Text) || s.From.Contains(textBoxSearch.Text)))
            ...
            if (rows.Any())
            {
                ...
                int prevRowsCount = db.Subjects.Include(s => s.subjectGroups).Where(s => s.id < prevId && (s.Name.Contains(textBoxSearch.Text) || s.To.Contains(textBoxSearch.Text) || s.Days.Contains(textBoxSearch.Text) || s.From.Contains(textBoxSearch.Text))).Count();
                int nextRowsCount = db.Subjects.Include(s => s.subjectGroups).Where(s => s.id > nextId && (s.Name.Contains(textBoxSearch.Text) || s.To.Contains(textBoxSearch.Text) || s.Days.Contains(textBoxSearch.Text) || s.From.Contains(textBoxSearch.Text))).Count();
        ...
        private void DisplayRows(List<Subject> rows)
        {
            ...
            foreach (var row in rows)
            {
                ...
                var schedule = "";
                schedule += row.From + " - " + row.To + " / ";
                string[] days = row.Days.Split(',');
                foreach (var day in days)
                {
                    schedule += day.Equals("Thursday")? day.Substring(0, 2) : day.ToUpper().Substring(0, 1);
                }
