...
namespace LCC_ENROLLMENT_SYSTEM.Components
{
    public partial class EnrollmentsTab : UserControl
    {
        ...
        public void GenerateCertificate2(int id)
        {
            AppDbContext db = new();
            ...
            document.AddSpaces(10f);
            //first table content
            PdfPTable table1 = new(2);
            table1.WidthPercentage = 100;
            table1.HorizontalAlignment = Element.ALIGN_LEFT;
            table1.AddCell(NewCell($"IDNO: {enrollment.student.student_id}",12,0,10));
            table1.AddCell(NewCell($"SCHOOL YEAR: {enrollment.schoolYear.ToString()}",12,0,10));
            table1.AddCell(NewCell($"NAME: {enrollment.student.ToString()}",12));
            table1.AddCell(NewCell($"YEAR/SEC: Grade {enrollment.gradeLevel.Level} {enrollment.section.Name}",12));
            table1.AddCell(NewCell($"Adviser: Mr/Ms. {enrollment.adviser.Firstname} {enrollment.adviser.Lastname}",12));
            document.Add(table1);
            ...
            table2.AddCell(NewCell("Schedule",12,iTextSharp.text.Font.BOLD));
            ...
            {
                var schedule = "";
                schedule += item.subject.From + " - " + item.subject.To + " ";
                string[] days = item.subject.Days.Split(',');
                foreach(var day in days)
                {
                    schedule += day.ToUpper().Substring(0, 1);
                }

                table2.AddCell(NewCell(item.subject.Name, 12));
                table2.AddCell(NewCell(schedule, 12));
            }

            document.Add(table2);

            document.AddSpaces(80,PdfDocument.Spacing.Before);
            float w = 30;
        ...
        public void GenerateCertificate(int id)
        {
            ...
            table1.AddCell(NewCell($"NAME: {enrollment.student.ToString()}", 12));
            table1.AddCell(NewCell($"YEAR/SEC: Grade {enrollment.gradeLevel.Level} {enrollment.section.Name}", 12));
            table1.AddCell(NewCell($"ADVISER: {enrollment.adviser.Firstname} {enrollment.adviser.Lastname}", 12,0,5,2));
            document.Add(table1);
